...
## BUG-D15: เอกสารหายเงียบถ้าปิด/รีเฟรชหน้าระหว่างอัปโหลดไฟล์แนบ — เจอตอน scrutinize 2026-07-14
**อาการ**: `_pendingFolderId`/`_pendingUrl`/`_attachments` เป็นแค่ตัวแปร JS ไม่ persist ที่ไหน ถ้าไฟล์หลักอัปโหลดสำเร็จ (โฟลเดอร์+ไฟล์สร้างแล้วใน Drive) แต่ปิดแท็บ/รีเฟรช/เน็ตหลุดระหว่างอัปโหลดไฟล์แนบ — ตัวแปรหายหมด โฟลเดอร์ค้างใน Drive แบบไม่มีใครรู้ตัว และ `queueForDailySummary()` ไม่เคยถูกเรียก (เรียกทีหลังสุดใน `notifyTeachers` หลังไฟล์แนบครบเท่านั้น) เอกสารจึงไม่เข้าคิว ไม่มีวันไปอยู่ในสรุปที่ส่งหา ผอ.เลย — เสี่ยงสูงขึ้นหลังทำมือถือ (D18) เพราะเบราว์เซอร์มือถือมักฆ่า/suspend แท็บพื้นหลัง
**แก้**: เก็บ `{docTitle, url, folderId}` ลง `sessionStorage` (key `sarabun_pending`) ตอนไฟล์หลักอัปโหลดสำเร็จ, ลบทิ้งตอนแจ้ง LINE สำเร็จหรือผู้ใช้เปลี่ยนไฟล์เอง — ตอนเปิดหน้าใหม่ถ้าเจอค้าง แสดง banner เตือนพร้อมลิงก์ไปโฟลเดอร์ ให้ผู้ใช้ไปเช็ค/อัปโหลดใหม่เอง (กู้ไฟล์แนบเดิมคืนอัตโนมัติไม่ได้ เพราะ browser ไม่ persist `File` object ข้าม reload — fix นี้แค่เปลี่ยนจาก "หายเงียบ" เป็น "แจ้งเตือน")
**สถานะ**: ✅ แก้แล้ว (index.html)

---

## BUG-D16: ไม่มี lock ตอนสร้างโฟลเดอร์ปี เสี่ยงสร้างซ้ำ — เจอตอน scrutinize 2026-07-14
**อาการ**: `getOrCreateFolder()` ไม่มี `LockService` (ต่างจาก `queueForDailySummary`/`sendDailySummary` ที่มีแล้ว) ถ้า 2 คนอัปโหลดเอกสารแรกของปีการศึกษาใหม่พร้อมกันพอดี ทั้งคู่เช็ค `hasNext()` ได้ `false` พร้อมกัน → สร้างโฟลเดอร์ปีเดียวกันซ้ำ 2 อัน เอกสารกระจายคนละโฟลเดอร์ สับสนตอนค้นหาย้อนหลัง (ไม่ถึงขั้นข้อมูลหาย)
**แก้**: ห่อทั้งฟังก์ชันด้วย `LockService.getScriptLock().waitLock(10000)` + `finally { releaseLock() }` เหมือน pattern ของ `queueForDailySummary`
**สถานะ**: ✅ แก้แล้ว (Code.gs)
...
### RISK-02: lock ชนกันตอน trigger 16:00 พอดี ทำให้ทั้งวันเลื่อนไปอีกวัน — เจอตอน scrutinize 2026-07-14
**อาการ**: ถ้ามีคนอัปโหลด (ถือ lock) พอดีตอน `sendDailySummary()` trigger ยิง (คลาดเคลื่อนได้ถึง ~1 ชม.) `waitLock(10000)` fail แล้ว `return` ทันที ไม่ retry วันเดียวกัน ต้องรอ trigger พรุ่งนี้
**ความเสี่ยง**: ต่ำมาก — ไม่มีข้อมูลหาย (คิวไม่ clear) แค่ ผอ. ได้รับสรุปช้า 1 วัน โอกาสชนต้องแคบมาก
**ตัดสินใจ**: ไม่แก้ตอนนี้ — เพิ่ม retry trigger ที่สองจะซับซ้อนเกินความคุ้มค่าเทียบกับ impact ที่ต่ำมาก
