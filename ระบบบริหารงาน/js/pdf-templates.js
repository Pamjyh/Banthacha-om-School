...
// ข้อความอ้างอิงกฎหมาย/ระเบียบใน Doc 4 (คำสั่งแต่งตั้งผู้กำหนด TOR) — จากไฟล์อ้างอิงจริง "4 คำสั่ง TOR.pdf"
// เป็นเลขคำสั่ง สพฐ./ระเบียบกระทรวงการคลังที่อ้างอำนาจแต่งตั้งกรรมการ ไม่ผูกกับรายการพัสดุแต่ละอัน
// (คำสั่งฉบับไหนก็อ้างอำนาจชุดเดียวกันนี้) เก็บเป็น constant กันพิมพ์ผิด/ลืมอัปเดตถ้าใช้ซ้ำใน Doc อื่น
...
async function generateDoc(docIndex, procItemId){
  ...
  if(docIndex === 4) return await generateDoc4(procItemId);
...
// รูปแบบอ้างอิงจากไฟล์จริง "4 คำสั่ง TOR.pdf" — ต่างจาก Doc 1/3 ("บันทึกข้อความ") ตรงที่นี่เป็นรูปแบบ
// "คำสั่ง" ทางการ: ไม่มีแถว ส่วนราชการ/เรียน, หัวเรื่องเป็น "คำสั่ง{ชื่อ ร.ร.} ที่ {เลขที่}" กึ่งกลางหน้า,
// วันที่อยู่ท้ายเอกสารข้างลายเซ็น ("สั่ง ณ วันที่...") ไม่ใช่บรรทัดบนแบบบันทึกข้อความ, ลงชื่อคนเดียว (ผอ.
// เท่านั้น — ไม่มีเจ้าหน้าที่/หัวหน้าเจ้าหน้าที่เหมือน Doc 3 เพราะนี่คือคำสั่งแต่งตั้งจาก ผอ. โดยตรง)
// ใช้ detail.date_order_tor (แถว "คำสั่งแต่งตั้งกรรมการ TOR" ใน PD_DATE_SEQUENCE) ไม่ใช่ date_approve_tor
// ของ Doc 3 — คนละวันที่กัน (Doc 3 = วันที่ขออนุมัติ, Doc 4 = วันที่ลงคำสั่งจริง)
// คณะกรรมการใช้ detail.committee_tor ชุดเดียวกับ Doc 3 (ไม่ต้องกรอกซ้ำ) แต่รูปแบบพิมพ์ต่างกัน — Doc 3
// พิมพ์บรรทัดเดียว "{ลำดับ}. {ชื่อ} ตำแหน่ง {ตำแหน่ง} {role}" ต่อคน ส่วน Doc 4 (ตามไฟล์จริง) แยกชื่อ+ตำแหน่ง
// เป็นรายคนก่อน แล้วต่อด้วยย่อหน้าอำนาจหน้าที่ร่วมกันทีเดียวหลังรายชื่อครบ (ไม่ใช้ role จาก DB เพราะ Doc 4
// ไม่แยก "ผู้กำหนดฯ"/"กรรมการ" เหมือน Doc 3 — ไฟล์อ้างอิงพิมพ์แค่ชื่อ+ตำแหน่งเรียงเป็นชุดเดียว)
// ⚠️ ข้อความอ้างอิงกฎหมายในไฟล์จริงใช้คำว่า "ครุภัณฑ์" (ของที่ใช้ได้นาน) แต่รายการตัวอย่างในไฟล์อ้างอิงเป็นการ
// "จัดจ้าง" (บริการ ไม่ใช่ครุภัณฑ์) — เปลี่ยนมาใช้คำว่า "พัสดุ" แทนในโค้ด (ครอบคลุมทั้งซื้อ/จ้าง ถูกต้องกว่า
// ตามระเบียบพัสดุจริง) ถ้า Pam เทียบกับเอกสารที่เคยใช้จริงแล้วเห็นว่าต้องคงคำว่า "ครุภัณฑ์" ไว้เป๊ะ แจ้งแก้ได้
async function generateDoc4(procItemId){
  const item = PROC.find(function(x){ return x.id === procItemId; });
  if(!item) return alert('ไม่พบรายการพัสดุนี้');
  ...
  if(!detail){ alert('กรุณาบันทึกข้อมูลในฟอร์ม "กรอกเอกสารพัสดุ" ก่อน แล้วค่อยพิมพ์เอกสาร'); return; }
  if(!detail.doc_number){ alert('ยังไม่มีเลขที่เอกสาร กรุณาบันทึกฟอร์มก่อน'); return; }
  if(!detail.date_order_tor){ alert('กรุณากรอก "วันที่คำสั่งแต่งตั้งกรรมการ TOR" ในฟอร์มก่อนพิมพ์เอกสารนี้'); return; }
  ...
  // คณะกรรมการ TOR — ใช้ข้อมูลชุดเดียวกับ Doc 3 (detail.committee_tor) แต่ไม่ใช้ field "role" ที่นี่
  // (ดูหมายเหตุด้านบน — Doc 4 พิมพ์แค่ชื่อ+ตำแหน่งเรียงกัน ไม่แยกบทบาทรายคนแบบ Doc 3)
  const torCommittee = (detail.committee_tor || [])
    .filter(function(c){ return c && c.staff_id; })
    .map(function(c){
      const s = (STAFF_LIST||[]).find(function(x){ return String(x.id) === String(c.staff_id); });
  ...
  const torRows = torCommittee.map(function(c, i){
    return '<div style="margin-top:2mm">'+(i + 1)+'. '+escHtml(c.name)+'</div>'+
      '<div>ตำแหน่ง '+escHtml(c.position)+'</div>';
  }).join('');

  const director = findDirector();
  const sigCenter = director
    ? 'ลงชื่อ .......................................<br>('+escHtml((director.prefix||'')+director.name)+')<br>ผู้อำนวยการ'+escHtml(SCHOOL_FULL_NAME)
    : 'ลงชื่อ .......................................';

  const html = '<!DOCTYPE html><html lang="th"><head><meta charset="UTF-8">'+
    '<title>'+escHtml((detail.doc_number||'doc').replace(/[\/\\]/g,'-')+'-'+PD_DOC_NAMES[4])+'</title>'+
    '<style>'+officialDocCss()+'</style></head><body>'+
    '<img class="garuda" src="data:image/jpeg;base64,'+GARUDA_B64+'">'+
    '<div class="doc-title">คำสั่ง'+escHtml(SCHOOL_FULL_NAME)+'<br>ที่ '+escHtml(bareDocNumber)+'</div>'+
    '<div class="doc-title2">เรื่อง แต่งตั้งผู้กำหนดรายละเอียดคุณลักษณะเฉพาะ (TOR หรือ Spec) การ'+buyOrHire+escHtml(itemTitle)+'</div>'+
    '<hr class="sep">'+
    '<p class="body-para">ด้วย'+escHtml(SCHOOL_FULL_NAME)+' จะดำเนินการ'+buyOrHire+escHtml(itemTitle)+' จำนวน '+itemCount+
      ' รายการ ภายในวงเงินทั้งสิ้นจำนวน '+fmt(totalAmount)+' บาท ('+thaiBahtText(totalAmount)+
      ') สำหรับใช้ในราชการของ'+escHtml(SCHOOL_FULL_NAME)+'</p>'+
    '<p class="body-para">ฉะนั้น '+TOR_ORDER_LEGAL_BASIS+'</p>'+
