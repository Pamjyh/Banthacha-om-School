...
function renderDashboard(){
  ...
  document.getElementById('db-projects-sub').textContent = `งบรวม ${numFull(totalBudget)} บาท`;
  document.getElementById('db-budget').textContent = numFull(totalBudget);
  ...
  document.getElementById('db-done-sub').textContent = numFull(totalDone.reduce((s,i)=>s+Number(i.amount||0),0))+' บาท';
