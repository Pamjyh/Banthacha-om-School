...
session   → sessionStorage['school_admin_session'] (หมดอัตโนมัติเมื่อปิด tab/browser — ไม่มี timer countdown)
...
- **Scrutinize + Bug fix (มิ.ย. 2569)**: ตรวจสอบระบบบริหารงานด้วย outsider review พบและแก้ 5 bugs:
  - [BLOCKER] delete.js — ลบ finance_transaction แล้ว revert procurement_item.withdraw_status ด้วย (ป้องกัน data inconsistency)
  - [MAJOR] export.js — แก้ `i.doc_no` → `i.document_no` (เลขเอกสารใน CSV ว่างมาตลอด)
  - [MAJOR] import.js — เพิ่ม `adminGuard()` ต้นฟังก์ชัน confirmImport() และ confirmExcelImport()
  - [MINOR] helpers.js — เพิ่ม escape `<` `>` `&` ใน escHtml(), ครอบทุก innerHTML user input ใน renderProc() และ renderTransactions()
  - [MINOR] auth.js + HANDOFF — แก้ comment "10 นาที timeout" ให้ตรงกับ behavior จริง (sessionStorage หมดเมื่อปิด tab)
...
### ✅ แก้แล้ว (มิ.ย. 2569): Scrutinize Round 2 — ทุกระบบ

**ระบบออมทรัพย์:**
- [BLOCKER] SW registration code อยู่นอก `</script>` tag → ย้ายเข้าใน script block
- [MAJOR] month filter ใช้ raw `t.date` (ISO string) เปรียบเทียบกับ Thai month name → แก้ routing ผ่าน `fmtDate()` ก่อน filter
- [MINOR] `loadAdmin()` swallow error → เพิ่ม error state พร้อมปุ่มลองใหม่

**ระบบค่ารถ:**
- [BLOCKER] SW registration code อยู่นอก `</script>` tag → ย้ายเข้าใน script block
- [MINOR] `esc()` ไม่ escape single quote `'` → เพิ่ม `&#39;`

**ระบบลงเวลา:**
- [BLOCKER] SW registration code อยู่นอก `</script>` tag → ย้ายเข้าใน script block
- [BLOCKER] Admin password hardcoded `'admin123'` ใน public GitHub source (client-side only) → เปลี่ยนเป็น SHA-256 hash ใน localStorage (`thatime_admin_hash`) — ครั้งแรกเข้าระบบตั้ง password เอง
- [MAJOR] XSS — `e.name`, `e.username`, `emp.name`, `emp.photo alt` render ตรงใน innerHTML โดยไม่ escape → เพิ่ม `esc()` helper ครอบ 9 จุด
- เพิ่ม: `esc()`, `hashPw()`, `ADMIN_PW_KEY` constant

**ระบบสารบัญ:**
- [BLOCKER] SW registration code อยู่นอก `</script>` tag → ย้ายเข้าใน script block
- [MINOR] `esc()` ไม่ escape single quote → เพิ่ม `&#39;`

**School Portal หน้าหลัก:** ไม่มี bug — ship as-is

> ⚠️ Pattern ที่พบซ้ำทุกระบบ: SW code ถูก copy-paste ต่อท้ายไฟล์นอก `</script>` tag — แก้ครบแล้วทุกระบบในรอบนี้

### ✅ แก้แล้ว (มิ.ย. 2569): Supabase RLS ระบบลงเวลา — Phase 1 + Phase 2

**Phase 1** (migration: `phase1_rls_security_fix`):
- REVOKE EXECUTE on LINE functions (`send_line_morning_summary`, `send_line_evening_summary`, `send_line_leave_result`) from anon + PUBLIC
- Fix `search_path` mutable warning บน 3 LINE functions
- DROP duplicate policy "allow delete leaves" บน leaves table
- ADD `admin_hash text` column ไปยัง `settings` table
- CREATE `admin_set_hash(p_new_hash, p_old_hash)` RPC (SECURITY DEFINER) สำหรับ first-run + password change

**Phase 2** (migration: `phase2_admin_rpc_tighten_rls`):
- CREATE 8 SECURITY DEFINER RPC functions ที่ validate hash ก่อน execute:
  - `_validate_admin_hash(text)` — helper
...
- DROP 7 open write RLS policies:
  - employees: INSERT/UPDATE/DELETE
  - attendance: DELETE
  - leaves: UPDATE/DELETE
  - settings: UPDATE
- Public ยังทำได้: attendance INSERT (เช็คอิน/เอาท์) + leaves INSERT (ยื่นใบลา)

**Client (ระบบลงเวลา/index.html)** — commit `9612fbd`:
- `doAdminLogin()` — sync hash ไป Supabase ทุกครั้ง login ผ่าน `admin_set_hash(h, h)` (handles first-run, re-sync on new device, reconfirm)
- `getH()` helper — return `localStorage.getItem(ADMIN_PW_KEY)`
- 7 admin ops เปลี่ยนจาก `sb.from().update/delete/insert` เป็น `sb.rpc('admin_*', {p_hash:getH(), ...})`
...
| git index.lock ค้าง | `mv .git/index.lock .git/index.lock.bak` (sandbox ลบตรงไม่ได้ — rename แทน) |
