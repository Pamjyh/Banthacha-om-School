...
| ระบบดูแลช่วยเหลือนักเรียน | `.../ระบบดูแลช่วยเหลือนักเรียน/` | Supabase (school-attendance project) |
...
- **Dark mode**: ปุ่มลอยมุมขวาล่าง (`#theme-toggle-btn`) กดสลับเอง จำค่าใน `localStorage['schoolPortalTheme']` ('dark'/'light', ไม่มีคีย์ = auto ตามเวลาเดิม ช่วงเดียวกับ badge "🌙 ตอนดึก") — override เฉพาะ CSS custom properties กลาง (`--canvas*`, `--ink*`, `--hairline`, `--primary*`, `--shadow*`) ผ่าน `html.dark-mode {...}` สีแบรนด์/ไอคอนแต่ละระบบคงเดิม
  ...
  - **✅ Polish รอบ 2 หลัง live-test จริง (2026-08-08)** — Pam ทดสอบแล้วฟีดแบ็ก: "สีตัดกันแรงเกินไป/แสบตา", "บางจุดยังสว่างจ้าอยู่", "โทนสีแต่ละระบบไม่กลมกลืนกับพื้นมืด" ระบุ 3 ระบบ: Portal, การเงินและพัสดุ, ดูแลช่วยเหลือนักเรียน
    - **root cause ใหม่ที่ต่างจาก scrutinize รอบก่อน**: ไม่ใช่ contrast ratio fail (ตัวอักษรอ่านออกปกติ) แต่เป็นปัญหา "พื้นที่ใหญ่พาสเทลสว่าง/อิ่มตัวถูกปล่อยไว้เหมือนเดิมตอน dark mode" — สมมติฐานเดิม "self-contained ปลอดภัย" ใช้ได้กับ chip/badge เล็กเท่านั้น พอเป็นพื้นที่ใหญ่ (แถบเต็มการ์ด/แถบเต็มความกว้าง/กริดปุ่มเต็มหน้าจอ) กลับดูจ้า/ไม่กลืนกับพื้นเกือบดำรอบข้างแทน
    - **Portal** (commit `f256276`): เพิ่ม dark-tinted `-bg`/`-hover` 6 สี (teal/green/orange/purple/blue/pink) ให้ `.card-icon-band` แต่ละระบบ hue เดิม luminance ต่ำ + เพิ่ม `--badge-on/off/soon/blue-text` แยกจากสีเดิมที่ออกแบบมาคู่กับพื้นสว่างเท่านั้น + แบนเนอร์วันสำคัญ (`SPECIAL_DAYS`) ทั้ง 6 รายการเพิ่ม `bgDark`/`colorDark` + แก้ `applyTheme()` ให้รีเฟรชสีแบนเนอร์ทันทีตอนสลับโหมด (เดิมค้างสีได้ถึง 60 วิ)
    - **การเงินและพัสดุ** (commit `5bdb15c`): กราฟ `external.js` (`renderExtCharts`) เดิม hardcode hex โหมดสว่างตรงๆ ทั้งที่ `--up`/`--signal` มี dark variant อยู่แล้วใน `css/styles.css` — เพิ่ม `extCssVar()` อ่าน `getComputedStyle` สดทุกครั้งที่ render ผูกบาร์รายรับ/รายจ่าย+จุด/เส้นกราฟสะสมเข้ากับตัวแปรจริง, เส้นขอบวงกลม donut/จุดกราฟเส้น (`#fff` cutout effect) → `var(--white)`, เพิ่ม `--chart-accent`/`--chart-accent-rgb` ใหม่ให้เส้นยอดสะสม (ไม่มี token กลางเดิม) — 12 สี `EXT_PALETTE` ของ donut ไม่แตะ (swatch/slice เล็ก contrast ผ่าน AA อยู่แล้วทุกสี)
    - **ดูแลช่วยเหลือนักเรียน** (commit `16d7cf5`): `.grade-header` (แถบหัวข้อกลุ่มชั้นเรียนเต็มความกว้าง) + `.pin-key` (กริดปุ่ม PIN 10-12 ปุ่มเต็มหน้าจอ login) ใช้พาสเทลม่วง `#f0eaf5` คงที่ทั้ง 2 โหมด → เพิ่ม `--grade-bg`/`--grade-bg-hover` dark variant คู่กับ `--pri-dark-text` (สลับจาก `--pri-dark` คงที่เดิม) + เพิ่ม `.stat-pill:hover` dark override ที่ขาดไป + เพิ่ม `--danger-bg`/`--msg-error-text`/`--ok-bg`/`--ok-text` ใหม่ให้ `.msg.error`/`.badge.health`/`.msg.ok` (ไม่เคยมี dark override เลย, **ระวัง**: ตั้งใจไม่ใช้ `--danger-text` เดิมตรงๆ เพราะจะทำให้ contrast โหมดสว่างตกจาก 4.76:1 เหลือ 3.34:1 — บทเรียนซ้ำจากบั๊ก `.quota-ok` ที่เจอตอน implement รอบแรกในระบบสารบัญ) — `.badge.temp`/`.badge` เริ่มต้นไม่แตะ (เป็น chip เล็กจริง `.7rem`/2-8px padding ต่างจาก grade-header/pin-key)
    - ทุกคู่สีใหม่ verify WCAG ผ่าน (Python คำนวณ relative luminance เหมือนรอบก่อน), verify `node --check` + brace balance ทุกไฟล์ที่แตะก่อน commit
    - **ยังไม่ได้ live-test รอบนี้** — รอ Pam push + เปิดดูจริงอีกรอบ โดยเฉพาะ 3 ระบบที่แก้ (Portal, การเงินและพัสดุ, ดูแลช่วยเหลือนักเรียน)
    - **scrutinize รอบ 3 (2026-08-08) พบ 2 findings แก้ครบแล้ว** (commit `efa6eff`) — trace `applyTheme()`/Chart.js cross-tab sync แบบ outsider:
      - F1 (MINOR, Portal): `updateGreeting()` เรียก `updateSpecialDayBanner()` ซ้ำ 2 ครั้งทุก 60 วิ เพราะ `applyTheme()` (เรียกบรรทัดบนมันเอง) มี call นี้อยู่แล้วตั้งแต่แก้บั๊กแบนเนอร์ค้างสีรอบ 2 — ไม่กระทบผล (idempotent) แค่สิ้นเปลือง → ลบบรรทัดซ้ำ
      - F2 (MINOR, ระบบบริหารงาน): กราฟ `external.js` ไม่รีเฟรชสีถ้าสลับ dark mode จาก Portal อีกแท็บขณะค้างอยู่แท็บ "กราฟ" เงินนอกพอดี (Chart.js วาด canvas บิตแมป ไม่ reactive ต่อ CSS var) → เพิ่ม `storage` event listener สั่ง `renderExtCharts()` ใหม่เฉพาะตอนอยู่แท็บกราฟจริง + โหลดข้อมูลแล้ว
      - **พบเพิ่มนอก scope**: ระบบดูแลช่วยเหลือนักเรียนบรรทัด ~454 ข้อความเตือนเบอร์โทรไม่ชัดใน `.call-option` ใช้ `var(--danger-text)` บนพื้น `#faf7fc` contrast 3.6:1 (ต่ำกว่า WCAG AA 4.5:1) — เป็นปัญหาเดิมทั้ง 2 โหมด ไม่ใช่ regression จากงาน dark mode ไม่ได้แตะ รอ Pam ตัดสินใจแยก
...
### LINE Bot (Supabase pg_cron) — อัปเดต 2026-07-10
...
> ✅ **Pivot 2026-07-10: เลิกใช้แผนกลุ่มไลน์ใหม่ทั้งหมด — เปลี่ยนไปส่ง push 1:1 หา ผอ. คนเดียวแทน**
> ปัญหาเดิม (บอทเข้ากลุ่มไม่ได้เพราะ LINE จำกัด 1 OA/กลุ่ม) หมดความจำเป็นต้องแก้แล้ว เพราะไม่ใช้กลุ่มอีกต่อไป — Edge Function `line-webhook-capture` + table `line_webhook_log` ที่สร้างไว้จับ groupId ตอนนั้นเลิกใช้แล้ว (ยังไม่ได้ลบทิ้ง ปลอดภัย ไม่กระทบอะไร)
> Pam มี LINE userId ของ ผอ. อยู่แล้ว (ผอ. แอดบอทเป็นเพื่อนไว้ก่อนหน้านี้) → ทดสอบ push จริงสำเร็จ 2026-07-10 11:47 (status 200, LINE message id `622145339356610935`) ก่อนเปลี่ยน production function → migration `line_evening_summary_to_principal_only` applied
...
### ตาราง (prefix `care_` กันชนกับตารางเดิม)
`care_students` (roster mirror, sync จากออมทรัพย์), `care_profiles` (ข้อมูลอ่อนไหวทั้งหมด, denormalized ตารางเดียว), `care_settings` (secrets/state ภายใน)

### RPC ทั้งหมด (password-gated, SECURITY DEFINER)
`care_login`, `care_get_students`, `care_upsert_profile`, `care_capture_pin`, `care_add_temp_student`, `care_merge_temp`, `care_trigger_sync`, `care_get_sync_status` — บวก internal `care_fire_sync_request`/`care_resolve_sync_response`/`care_sync_roster_from_gas` (ไม่ expose ให้ anon เรียกตรง ยกเว้น trigger_sync)

### Sync จากระบบออมทรัพย์
ยิง GET ตรงไปที่ GAS Web App ออมทรัพย์ (`?action=getStudents`) ผ่าน `pg_net` — **ไม่ต้องแก้ Code.gs ออมทรัพย์เลย** เพราะ `getStudents` ไม่เช็ครหัสผ่าน (พบระหว่างอ่านโค้ดจริง) — two-phase async (ยิง→resolve รอบถัดไป) เพราะ anon statement_timeout ~3s ไม่พอรอ synchronous — `pg_cron` ทุก 5 นาที (`care_sync_roster_every_5_min`)

### 🐛 บั๊กที่เจอ+แก้ระหว่าง scrutinize (2026-07-09)
- **[BLOCKER]** `care_resolve_sync_response` เดิม delete pending marker ก่อน raise exception — PL/pgSQL rollback ทั้ง transaction รวม delete ด้วย ทำให้ค้าง infinite retry loop ถ้า GAS ตอบ response เพี้ยน แก้โดยเปลี่ยนแนวทางทั้งหมด: **ไม่ raise exception อีกต่อไป** เขียน error ลง `care_settings` (`last_sync_error`/`last_sync_error_at`) แล้ว return ปกติแทน (เช็คได้ผ่าน `care_get_sync_status` RPC หรือ query ตรง) — บทเรียนสำคัญ: nested BEGIN/EXCEPTION ป้องกันได้แค่ rollback เฉพาะ block ถ้าสุดท้ายยัง re-raise ออกไป transaction ทั้งหมดก็ยัง abort อยู่ดี ต้องเลิก raise จริงๆ ถึงจะรอด
- **[MAJOR]** `care_login` เดิมไม่มี rate limit เลย เพิ่ม lockout 5 ครั้งผิด → ล็อก 15 นาที เก็บ state ใน `care_settings` (`login_fail_count`/`login_locked_until`) — ระหว่างเขียนเจอบั๊กซ้อน: `SELECT ... INTO var` เมื่อไม่เจอแถวจะ set NULL เสมอไม่สนใจ COALESCE ในนั้น (เพราะไม่มีแถวให้ประเมิน) ทำให้ครั้งแรกนับพลาดเป็น NULL+1=NULL ตลอด ต้องเช็ค `if var is null then var:=0` แยกหลัง select เสมอ
- **[MAJOR]** `care_merge_temp` (จับคู่นักเรียนใหม่กับ sync) — ถ้า real_id ยังไม่เคยมีแถว `care_profiles` มาก่อน UPDATE...FROM จะ match 0 แถวเงียบๆ ข้อมูลจาก temp หายไม่มี error แก้โดย insert แถวเปล่าให้ real_id ก่อนเสมอ + เพิ่ม confirm dialog โชว์ชื่อ+ชั้นทั้ง 2 ฝั่งก่อนยืนยัน (กันเลือกผิดคนจาก dropdown 237 คน)
- **[MINOR]** รหัสผ่านเปลี่ยนจากเก็บ sessionStorage เป็น memory เท่านั้น (เข้มกว่าระบบออมทรัพย์ เพราะข้อมูลอ่อนไหวกว่า) + เพิ่ม soft warning เลขบัตรประชาชนไม่ครบ 13 หลัก

### 🐛 รอบ 2 — scrutinize ซ้ำหลังแก้รอบแรก (2026-07-09, ตามที่ Pam ขอให้ตรวจซ้ำก่อน push)
- **[MAJOR]** `care_fire_sync_request` / `care_resolve_sync_response` / `care_sync_roster_from_gas` ตอนสร้างลืม revoke สิทธิ์ default — PostgreSQL ให้ `EXECUTE` แก่ `PUBLIC` อัตโนมัติเสมอถ้าไม่ revoke ทำให้ anon (ที่มี anon key ฝังอยู่ใน frontend JS สาธารณะ) เรียกฟังก์ชัน sync ภายในตรงๆ ได้โดยไม่ต้องรู้รหัสผ่านเลย แก้ด้วย `revoke all ... from public, anon, authenticated` เหลือแค่ postgres/service_role (pg_cron ยังทำงานปกติ ยืนยันแล้ว)
- **[MAJOR]** `care_upsert_profile` เดิมไม่มีการเช็ค concurrent edit — frontend ส่งฟอร์มทั้งหมดทุกครั้งที่บันทึก (ไม่ใช่แค่ field ที่เปลี่ยน) ถ้าครู 2 คนเปิดโปรไฟล์เด็กคนเดียวกันใกล้ๆ กัน คนที่เซฟทีหลังจะเขียนทับข้อมูลของคนแรกแบบเงียบๆ (เช่น ข้อมูลแพ้ยาที่เพิ่งกรอกหายไป) แก้ด้วย optimistic concurrency: เพิ่ม parameter `p_expected_updated_at` เทียบกับ `updated_at` จริงใน DB ก่อนบันทึก ถ้าไม่ตรง (มีคนแก้ไปแล้ว) ปฏิเสธพร้อมข้อความชัดเจนให้โหลดหน้าใหม่ — ทดสอบยืนยันแล้วว่า block การเขียนทับจริง
  - 🐛 **บั๊กเสริมระหว่างแก้จุดนี้**: เพิ่ม parameter ด้วย `CREATE OR REPLACE` คิดว่าจะทับฟังก์ชันเดิม แต่ Postgres นับ signature จากจำนวน+ชนิด parameter — ผลคือมี `care_upsert_profile` 2 overload ค้างพร้อมกัน (4-arg เดิมที่ไม่มี concurrency check + 5-arg ใหม่) ตัวเก่ายังเรียกได้ปกติเป็นช่องทางบายพาส ต้อง `drop function` ตัวเก่าทิ้งด้วยมือ — บทเรียน: เพิ่ม parameter ให้ RPC ที่มีอยู่แล้วต้องเช็ค `\df` /overload ซ้ำเสมอ อย่าสมมติว่า CREATE OR REPLACE จะทับให้เอง

### ทดสอบแล้ว (ผ่าน SQL จริงทุกจุด)
Sync จริงจากออมทรัพย์ได้ 237 คนถูกต้อง (เช็คชั้น ป.1 ครบ 34 คนตรงกับที่ควรเป็น), round-trip upsert/read โปรไฟล์, ปักหมุด GPS + ลิงก์ Google Maps, temp-student + merge flow, rate limit lockout/unlock, error-log ไม่ raise แล้ว cleanup รอดจริง, internal sync functions บล็อก anon เรียกตรงแล้ว (ยืนยันด้วย `set role anon` จริง), optimistic concurrency กันเขียนทับข้อมูลจริง

### 🔴 ค้างอยู่ (รอ Pam)
- **Commit สำเร็จแล้วทั้งหมด** (`625c4ce` + `d5d8c06`) แต่ **push ไม่ได้จาก sandbox** (ไม่มี GitHub credentials) — **Pam ต้องรันเองใน Terminal เครื่องจริง**:
  ```bash
  cd ~/Documents/ระบบงานโรงเรียนบ้านท่าชะอม/"School Portal"
  git push --force origin main
  ```
- **ยังไม่ได้ทดสอบจริงในเบราว์เซอร์** — ทุกอย่างข้างบนทดสอบผ่าน SQL/backend เท่านั้น Pam ต้องเปิดเว็บจริงหลัง push แล้วลองกรอกโปรไฟล์นักเรียนจริง 1 คน + ลองปักหมุด GPS ตอนอยู่หน้าบ้านเด็กจริง เพื่อยืนยัน end-to-end
