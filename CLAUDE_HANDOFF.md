...
**เป้าหมาย**: หน้ารวมทางเข้า 5 ระบบ — static ล้วน ไม่มี backend ของตัวเอง ไม่ต้อง login เพื่อใช้งานปกติ

### ✅ ปิด/เปิดระบบชั่วคราว (system-status) — 2026-07-04
- ไฟล์ `system-status.js` (root ของ School Portal) — object `window.SYSTEM_STATUS = { finance, savings, attendance, sarabun, karod: "on"/"off" }`
- **วิธีปิดระบบชั่วคราว**: แก้ค่าเป็น `"off"` ในไฟล์นี้ → `git push` → มีผลทันที ไม่ต้อง deploy อะไรเพิ่ม (ตั้งใจเลือกไม่ให้มี backend ใหม่ — ดู decision เต็มในบทสนทนา grill)
- Card ในหน้า Portal จับคู่กับ config ผ่าน `data-sys-id` (`finance`/`savings`/`attendance`/`sarabun`/`karod`) — ตอนโหลดหน้า ดึงไฟล์นี้มาเช็คก่อน ค่อยผูกปุ่มกด (กัน bug ที่ปุ่มกดใช้ได้ทั้งที่ badge บอกว่าปิด)
- `off` = badge ส้ม "ปิดปรับปรุงชั่วคราว" + กดเข้าไม่ได้ + aria-label บอกด้วย (screen reader)
- **Fail-open ทุกจุด**: โหลดไฟล์ไม่ได้ / ไม่มี id นี้ / พิมพ์ผิด → ถือว่า `"on"` เสมอ กันพลาดล็อกทุกระบบพร้อมกันเพราะ typo
- `sw.js` แก้เพิ่ม `NO_CACHE_PATHS` ให้ `system-status.js` ไม่โดน cache แบบ stale-while-revalidate (ของเดิมใช้กับทุกไฟล์ในโดเมนตัวเอง รวม index.html เอง) — ไม่งั้น toggle จะช้าไป 1 รอบเปิดเว็บเสมอ; ฝั่ง index.html โหลดไฟล์ด้วย cache-busting URL (`?t=timestamp`) เป็นตัวกันหลัก
