...
### 🔴 ค้างอยู่ (รอ Pam)
- **Push ไม่สำเร็จจาก sandbox** — commit `625c4ce` (โฟลเดอร์ระบบใหม่) push ขึ้นไปสำเร็จแล้วในเชิง commit แต่ค้าง `index.lock` ระหว่างพยายาม commit การ์ด Portal (`School Portal/index.html` แก้ไขค้างอยู่ ยังไม่ commit) — **Pam ต้องรันเองใน Terminal เครื่องจริง**:
  ```bash
  cd ~/Documents/ระบบงานโรงเรียนบ้านท่าชะอม/"School Portal"
  git add index.html
  git commit -m "Portal: เพิ่มการ์ดที่ 6 ระบบดูแลช่วยเหลือนักเรียน"
  git push --force origin main
  ```
  (commit `625c4ce` ที่มีอยู่แล้วจะถูก push ไปด้วยในจังหวะเดียวกัน)
