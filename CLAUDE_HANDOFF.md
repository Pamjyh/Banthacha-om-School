...
**เป้าหมาย**: แสดงรายชื่อ+รูป+ตำแหน่งบุคลากรโรงเรียน อยู่บนสุดของหน้า Portal (พับเก็บ) — public ไม่ต้อง login เพื่อดู มีหน้า Admin แยกให้เพิ่ม/แก้/ลบเอง
**Stack**: HTML/JS (หน้า Admin แยกโฟลเดอร์) → Supabase project `cgwtgqyllalaogdgyxlo` (project เดียวกับระบบลงเวลา — ไม่ใช่ project ใหม่)
**Design เต็ม**: `BLUEPRINT-staff-directory.md` + `CONSTRUCTION_PLAN-staff-directory.md` (root ของ `ระบบงานโรงเรียนบ้านท่าชะอม/`)

### Backend
- ตาราง `staff_directory` (`id, full_name, position, photo, display_order, head_of, created_at, updated_at`) — RLS: anon select-only, เขียนได้เฉพาะผ่าน RPC
...
- RPC (`SECURITY DEFINER`, รหัสผ่าน hardcode ในฟังก์ชัน — **ไม่มีในไฟล์ใดๆ ที่ push**): `staff_directory_verify_password`, `staff_directory_add`, `staff_directory_update`, `staff_directory_delete`
  - `staff_directory_update` ใช้ `photo = coalesce(p_photo, photo)` — กันรูปหายถ้า frontend ไม่ส่งรูปใหม่มาตอนแก้ไข (จุดเสี่ยงที่สุดของฟีเจอร์นี้ ทดสอบแล้วผ่าน)
- Auth แบบง่าย (ไม่ hash/rate-limit) — ยอมรับความเสี่ยงเพราะข้อมูลเป็น public data อยู่แล้ว เหมือน `portal_toggle_status`

### Frontend
- `School Portal/ทำเนียบบุคลากร/index.html` — หน้า Admin (login รหัสผ่าน + เพิ่ม/แก้ไข/ลบ + ย่อรูปด้วย canvas ก่อนแปลง base64)
- `School Portal/index.html` — ส่วนแสดงผล read-only เท่านั้น (แถบพับเก็บ "👥 ทำเนียบบุคลากร ▾" เริ่มพับ, fetch ตอนโหลดหน้า, fail-safe ซ่อนทั้ง section ถ้าโหลดพัง)

### สถานะ (2026-07-09)
- ✅ Schema + RLS + RPC ทดสอบครบ (รหัสผ่านผิด/ถูก, แก้ไขไม่ทำรูปหาย, id ผิด)
- ✅ Seed ชื่อ+รูปจริงครบ 17 คน จากตาราง `employees` ของระบบลงเวลา (copy ครั้งเดียว ไม่ใช่ live link — แก้ที่นี่ไม่กระทบ `employees` และกลับกัน)
- ✅ หน้า Admin (login, เพิ่ม, แก้ไข/ลบ) + ส่วนแสดงผลใน Portal — ทดสอบจริงกับ Pam ผ่านทุก stage
- ✅ Service Worker cache บัมพ์เป็น `banthacha-om-v3` แล้ว
- 🟡 **ตำแหน่ง/หัวหน้าฝ่ายของทั้ง 17 คนยังเป็น placeholder "ยังไม่ระบุ"** — Pam จะเข้าไปแก้เองทีละคนผ่านหน้า Admin ทีหลัง (ไม่ block การ ship)
